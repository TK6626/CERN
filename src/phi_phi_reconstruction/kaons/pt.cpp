...
// !./bashing/run_file.sh phi_phi_reconstruction/kaons/pt
...
int main() {
	...
	for (TString topo : topology) {
		...
		TH1F* hist_both = new TH1F("hist_both", "", 200, 0, 2);
		...
		df1.Foreach(
			[&hist_both] (const RVecLorCyl p) {	
				...
				hist_both->Fill(p4.Pt());
				}
			}, {"kaon_four_momentum"}
		);
				
		hist_both->SetTitle(TString::Format("trk_pt;Events  [%.3g]", hist_both->GetXaxis()->GetBinWidth(1)));
	
		RVecDraw dat = {{hist_both, "HIST"}};
		
		SaveCanvas(c1, dat, TString::Format("media/root_files/phi_phi_reconstruction/kaons/pt/"+topo+"/mass_cut=%.4gMeV.root", mass_bound*1e3), "RECREATE");
