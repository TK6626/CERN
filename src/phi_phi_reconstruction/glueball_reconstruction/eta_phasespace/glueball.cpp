...
int main() {
	...
	for (TString topo : topology) {
		...
		RN df = df_df.Filter(
				[&low_eta, &high_eta](const RVecF eta) {
	...
	// Combine signal and background
		...
		SaveCanvas(can, dat, TString::Format("media/root_files/phi_phi_reconstruction/glueball_reconstruction/eta_phasespace/"+ topo +"/mass_bound=%.3g_MeV.root", mass_bound*1e3), "RECREATE");
