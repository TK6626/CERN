...
// !./bashing/run_file.sh phi_phi_reconstruction/glueball
...
int main() {
	...
	for (TString topo : topology) {
		...
		TH1F* h = new TH1F("h", "h", 100, 2000, 3000);	
		...
		SaveCanvas(c, dat, TString::Format("media/root_files/phi_phi_reconstruction/glueball_reconstruction/"+topo+"/and/mass_bound=%.3g_MeV.root", mass_bound*1e3), "RECREATE");
