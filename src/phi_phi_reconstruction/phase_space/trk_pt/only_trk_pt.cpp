...
// !bashing/run_file.sh phi_phi_reconstruction/phase_space/trk_pt/only_trk_pt
...
int main() {
	...
	const RVecStr topologies = {"20, 40"};
	...
	for (TString topo : topologies) {
		...
		for (Float_t val : mass_cuts) {
			...
			const TString outname = TString::Format("media/root_files/phi_phi_reconstruction/phase_space/mass_cuts/%s/compare_topologies/" + topo + "/only_pt/mass_bound = %.3g MeV.root", branch, val*1e3);
