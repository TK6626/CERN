...
int main() {
	...
	for (TString topo : topology) {
		...
		RooRealVar sigma_1("sigma_1", "width of gauss 1", 0.5, 0.1, 3);
		...
		RooAbsPdf* model = BiGaussianPdf("model", "signal gaussian", x_var, mean_1, sigma_1, mean_2, sigma_2, frac);
	
		//RooRealVar nsig("nsig", "signal yield", hist->Integral() * 0.9, 0, hist->Integral());
