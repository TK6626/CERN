...
// !g++ src/reconstruct_all_data/rho_reconstruction/x_projection_breit_wigner.cpp lib/*.cpp -Llib -ldict `root-config --cflags --libs` -o bin/reconstruct_all_data/rho_reconstruction/x_projection_breit_wigner; ./bin/reconstruct_all_data/rho_reconstruction/x_projection_breit_wigner
...
int main() {
	...
	Double_t y_1_40 = lower_bound, y_2_40 = upper_bound; // MeV/c^2
...
// relativistic breit-wigner formula
	...
	// Fit range
	Double_t fy_1 = 500, fy_2 = 850; // what part of the projection is actully fitted to 

	TF1* fit_x_20 = new TF1("fit_x_20", fit_relativistic_breit_wigner, fy_1, fy_2, 4);
	fit_x_20->SetParameters(760, 100, 10e14, 200);
	fit_x_20->SetParNames("m","Γ", "A_0", "C");
	fit_x_20->SetParLimits(0, 745, 770);
	fit_x_20->SetParLimits(1, 100, 180);
	...
	TF1* fit_x_40 = new TF1("fit_x_40", fit_relativistic_breit_wigner, fy_1, fy_2, 4);
	fit_x_40->SetParameters(750, 100, 5e14, 300);
	...
	x_projection_20->SetAxisRange(200, 1500, "Y");

	canvas_x->Update();
	TFile* outFile_x = new TFile("media/root_files/Younes_NTuples_TOTEM20_and_40/glueball_mass_reconstruction/rho_reconstruction/kaons_from_rho_x_proj_all_20_and_40.root", "RECREATE");
	canvas_x->Write();
	outFile_x->Close();
