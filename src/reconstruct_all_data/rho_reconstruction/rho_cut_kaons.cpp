...
//  !g++ src/reconstruct_all_data/rho_reconstruction/rho_cut_kaons.cpp lib/*.cpp -Llib -ldict `root-config --cflags --libs` -o bin/reconstruct_all_data/rho_reconstruction/rho_cut_kaons; ./bin/reconstruct_all_data/rho_reconstruction/rho_cut_kaons
...
df_40 = cut_branch(df_40, "trk_dz_snr", Z_constraint, true);
    ...
    TH2F* histo_20 = new TH2F("histo_20", "Invariant Mass;M_{1} (MeV/c^{2});M_{2} (MeV/c^{2})", 500, 300, 1300, 500, 300, 1300);
    df_20.Foreach(
    ...
    TH2F* histo_40 = new TH2F("histo_40", "Invariant Mass;M_{1} (MeV/c^{2});M_{2} (MeV/c^{2})", 500, 300, 1300, 500, 300, 1300);
    df_40.Foreach(
    ...
    histo_40->Draw("SAME");
    can1->Update();
    TFile* ouf = new TFile("media/root_files/Younes_NTuples_TOTEM20_and_40/glueball_mass_reconstruction/rho_reconstruction/mass_reconstruction.root", "RECREATE");
    can1->Write();
    ouf->Close();

    // Save histograms as ROOT files
    TFile* f20 = new TFile("media/root_files/Younes_NTuples_TOTEM20_and_40/glueball_mass_reconstruction/rho_reconstruction/histo_20.root", "RECREATE");
    histo_20->Write();
    f20->Close();

    TFile* f40 = new TFile("media/root_files/Younes_NTuples_TOTEM20_and_40/glueball_mass_reconstruction/rho_reconstruction/histo_40.root", "RECREATE");
    histo_40->Write();
    f40->Close();
