...
// !./bashing/run_file.sh reconstruct_all_data/rho_reconstruction/elastic_background/cut_low_pt
...
int main() {
	...
	const Float_t mass_bound = mass_bound_list[index]; // MeV	
	...
	TLegend* leg = new TLegend(0.8,0.55,  1,1, "Select pr_pt Cuts (MeV/c)");
	...
	for (const Float_t val : pt_cuts) {
		...
		TH1F* h_pt = new TH1F(TString::Format("hist_cut_%d", i), "h_pt", n_bins, 1200, 2400);
		
		auto df_cut = df.Filter([val] (const Double_t px_a, const Double_t py_a, const Double_t px_b, const Double_t py_b) {
			Bool_t a = (px_a*px_a + py_a*py_a) > val*val;
			Bool_t b = (px_b*px_b + py_b*py_b) > val*val;
			return a && b;
	...
	SaveOnlyCanvas(c, TString::Format("media/root_files/Younes_NTuples_TOTEM20_and_40/glueball_mass_reconstruction/rho_reconstruction/elastic_background/transverse_momentum/pt_" + topology + "/low_pr_pt_cut_mass_bound = %.4g MeV_c^2.root", mass_bound), "RECREATE");
