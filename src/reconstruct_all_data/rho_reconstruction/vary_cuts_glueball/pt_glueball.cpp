...
// !./bashing/run_file.sh reconstruct_all_data/rho_reconstruction/vary_cuts_glueball/pt_glueball

void fillHistFromP4(const RVecLor& p4, TH1* hist, Float_t m_rho, Float_t mass_bound);

/** make cuts on data to 
 * see how varying pt effects the (as so far of yet nonexistant peak at 2220Mev)
 */
int main() {
	...
	// pt has approx gaus distro near the mean=0, falls of sharply at pt =±3
    RVecF cuts = {100, 70, 55, 48, 40};
	cuts /=1e2;

	TH1F* hist = new TH1F("hist", "Invarient mass;M (MeV/c^{2});Events", num_bins[index], lower_limit[index], 2500);
	TCanvas* c = new TCanvas("c", "Canvas", 800, 600);
	TLegend* leg = new TLegend(0.7,0.4, 0.9,0.9, "selected cuts (GeV/c)");
	auto stack = new THStack();
    	
	const char* trk_var = "trk_pt";
    ...
    auto df_filtered = cut_branch(df, trk_var, VectorConstraint<Float_t>{
        .vector_condition = [val](Float_t pt) { return TMath::Abs(pt) < val; },
        .op = Operator::all,
        .element_count = 0
    }, false);
	...
	SaveOnlyCanvas(c, TString::Format("media/root_files/Younes_NTuples_TOTEM20_and_40/glueball_mass_reconstruction/rho_reconstruction/vary_cuts_glueball/pt_" + topology + "/%s mass_bound = %.4g MeV_c^2.root", trk_var, mass_bound), "RECREATE");
