...
//  !./bashing/run_file.sh reconstruct_all_data/rho_reconstruction/vary_cuts_glueball/p_glueball

void fillHistFromP4(const RVecLor& p4, TH1* hist, Float_t m_rho, Float_t mass_bound);

/** make cuts on data to 
 * see how varying p effects the (as so far of yet nonexistant peak at 2220Mev)
 */
int main() {
	...
	// p has approx gaus distro near the mean=0, falls of sharply at p =±3
    RVecF cuts = {500, 180, 120, 80, 55};
	cuts /=1e2;
    ...
    auto df_filtered = cut_branch(df, trk_var, VectorConstraint<Float_t>{
        .vector_condition = [val](Float_t p) { return TMath::Abs(p) < val; },
        .op = Operator::all,
        .element_count = 0
    }, false);
	...
	SaveOnlyCanvas(c, TString::Format("media/root_files/Younes_NTuples_TOTEM20_and_40/glueball_mass_reconstruction/rho_reconstruction/vary_cuts_glueball/p_" + topology + "/%s mass_bound = %.4g MeV_c^2.root", trk_var, mass_bound), "RECREATE");
