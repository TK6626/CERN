...
// !./bashing/run_file.sh reconstruct_all_data/rho_reconstruction/vary_cuts_glueball/zPV_glueball

void fillHistFromP4(const RVecLor& p4, TH1* hist, Float_t m_rho, Float_t mass_bound);

/** make cuts on data to 
 * see how varying zPV effects the (as so far of yet nonexistant peak at 2220Mev)
 */
int main() {
	...
	// zPV has approx gaus distro near the mean=0, falls of sharply at zPV =±3
    RVecF pt_cuts = {15, 7, 4, 2, 1};
    ...
    auto df_filtered = cut_branch(df, trk_var, VectorConstraint<Float_t>{
        .vector_condition = [val](Float_t zPV) { return TMath::Abs(zPV) < val; },
        .op = Operator::all,
    }, false);
	...
	SaveOnlyCanvas(c, TString::Format("media/root_files/Younes_NTuples_TOTEM20_and_40/glueball_mass_reconstruction/rho_reconstruction/vary_cuts_glueball/zPV_" + topology + "/%s mass_bound = %.4g MeV_c^2.root", trk_var, mass_bound), "RECREATE");
