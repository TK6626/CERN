...
//  !./bashing/run_file.sh reconstruct_all_data/kaon_reconstruction/vary_cuts_glueball/dxy_glueball
...
int main() {
    ...
    df_filtered.Foreach([&](const RVecLor& p4) {
        fillHistFromP4(p4, hist_cut, m_kaon_0, mass_bound);
    }, {"four_momentum"});
	...
	SaveOnlyCanvas(c, TString::Format("media/root_files/Younes_NTuples_TOTEM20_and_40/glueball_mass_reconstruction/kaon_reconstruction/vary_cuts_glueball/dxy_" + topology + "/%s mass_bound = %.4g MeV_c^2.root", trk_var, mass_bound), "RECREATE");
