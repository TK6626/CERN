...
int main(){
	...
	// load in all 20 type events ,get column names, enable multithreading
	...
	fit_y->Draw("SAME");
	canvas->Update();
	canvas->SaveAs("media/photos/Younes_NTuples_TOTEM20/glueball_mass_reconstruction/kaons_from_rho_y_proj_all_20.pdf");
	canvas->Clear();
	
	TFile* outFile_y = new TFile("media/root_files/Younes_NTuples_TOTEM20/glueball_mass_reconstruction/kaons_from_rho_y_proj_all_20.root", "RECREATE");
	y_projection->Draw("E*");
	fit_y->Draw("SAME");
	legend_y->Draw();
    canvas->Update();
	...
	fit_x->Draw("SAME");
	canvas->Update();
	canvas->SaveAs("media/photos/Younes_NTuples_TOTEM20/glueball_mass_reconstruction/kaons_from_rho_x_proj_all_20.pdf");
	canvas->Clear();
	
	TFile* outFile_x = new TFile("media/root_files/Younes_NTuples_TOTEM20/glueball_mass_reconstruction/kaons_from_rho_x_proj_all_20.root", "RECREATE");
	x_projection->Draw("E*");
	fit_x->Draw("SAME");
    canvas->Update();
	canvas->Write();
