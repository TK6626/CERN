...
//	Command to run file
//	!g++ src/reconstruct_all_data/trk_zPV.cpp lib/*.cpp -Llib -ldict `root-config --cflags --libs` -o bin/reconstruct_all_data/trk_zPV; ./bin/reconstruct_all_data/trk_zPV

/**
 * Look at cuts on the zPV interaction point?
...
void zPV_zPVerr_plot(RN df, TH1F* h);
...
int main(){
	...
	Float_t c_x1 = -10;
	Float_t c_x2 = 10;
	TH1F* h_20 = new TH1F("h_20", "dzPV / dzPV_{err} with 3#sigma bounds", 80, c_x1, c_x2);
	...
	zPV_zPVerr_plot(df_20, h_20);
	zPV_zPVerr_plot(df_40, h_40);

	h_20->GetXaxis()->SetTitle("#frac{zPV}{zPV_{err}}");
	h_20->GetYaxis()->SetTitle(Form("Events / bin width = %.2f", (double)h_20->GetXaxis()->GetBinWidth(1)));

	Float_t x1_fit = -6; 	// no units 
	Float_t x2_fit = 6;		// no units

	// Create a TF1 for h_20
	TF1* fit_20 = new TF1("fit_20", fit_gaussian, x1_fit, x2_fit, 4);
	fit_20->SetParameters(0, 6, 60e3, 5e3);
	fit_20->SetParNames("Mean", "Sigma", "Normal", "C");
	fit_20->SetParLimits(0, 0, 1);
	fit_20->SetParLimits(1, 3, 8);
	...
	fit_40->SetParameters(0, 6, 60e3, 5e3);
	fit_40->SetParNames("Mean", "Sigma", "Normal", "C");
	fit_40->SetParLimits(0, 0, 1);
	fit_40->SetParLimits(1, 3, 8);
	...
	TFile* f = new TFile("media/root_files/Younes_NTuples_TOTEM20_and_40/glueball_mass_reconstruction/trk_zPV_cut.root", "RECREATE");
	c->Write();
	f->Close();
	return 0;
}
