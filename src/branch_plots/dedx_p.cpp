...
// command
// !g++ src/branch_plots/dedx_p.cpp lib/*.cpp -Llib -ldict `root-config --cflags --libs` -o bin/branch_plots/dedx_p; ./bin/branch_plots/dedx_p
...
{  
  ...
  const char* tree_name = "tree";	
    ...
    TH2F *histo_20 = new TH2F("histo_20", "dE/dx vs p;Momentum (MeV);dE/dx (MeV/cm)",
                               1000, 0, 2.5,  // x-axis bins and range
                               1000, 0, 10); // y-axis bins and range
    TH2F *histo_40 = new TH2F("histo_40", "dE/dx vs p;Momentum (MeV);dE/dx (MeV/cm)",
	...
	c1->Update();	
	TFile* f20 = new TFile("media/root_files/Younes_NTuples_TOTEM20_and_40/branch_plots/dedx_p_20.root", "RECREATE");
    histo_20->Write();
    f20->Close();	
	c1->Clear();
    ...
    histo_40->Draw("COLZ");
	c1->Update();	
    TFile* f40 = new TFile("media/root_files/Younes_NTuples_TOTEM20_and_40/branch_plots/dedx_p_40.root", "RECREATE");
    histo_40->Write();
