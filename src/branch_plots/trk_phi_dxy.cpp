...
int main()
{
    ...
    Int_t phi_bins = 200;
    Int_t dxy_bins = 200;
    Float_t phi_min = -pi; // radians
    Float_t phi_max = pi; // radians
    Float_t dxy_min = -0.1; // 
    Float_t dxy_max = 0.1; // 

    // Create histogram
    TH2F* graph = new TH2F("graph", "XY Distance against phi", dxy_bins, dxy_min, dxy_max, phi_bins, phi_min, phi_max);
    graph->GetXaxis()->SetTitle("xy distance to IP");
	...
	df.Foreach(
	[graph]
	(const Int_t track_num, const RVecF &dxy, const RVecF &err, const RVecF &phi)
	{
		for (Int_t i = 0; i < track_num; ++i){
			graph->Fill(dxy[i], phi[i]);
		};
	}, {"ntrk", "trk_dxy", "trk_dxyerr", "trk_phi"}
	);
	...
	graph->Draw("COLZ");
	canvas->Update();
    canvas->SaveAs("media/photos/Younes_NTuples_TOTEM20/dxy_phi.pdf");
	canvas->Clear();

	// Save canvas as a root File
	TFile* outFile = new TFile("media/root_files/Younes_NTuples_TOTEM20/dxy_phi.root", "RECREATE");
	gStyle->SetPalette(1);
   	graph->Draw("surf1");
    canvas->Update();
	canvas->Write();
