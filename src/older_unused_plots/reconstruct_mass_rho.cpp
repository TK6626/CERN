...
int main(){
	...
	//Y Projection
	Double_t x_1 = 650, x_2 = 950; // MeV/c^2
	...
	fit_y->Draw("SAME");
	canvas->Update();
	canvas->SaveAs("media/photos/Younes_NTuples_TOTEM20/glueball_mass_reconstruction/rho_y_proj.pdf");
	canvas->Clear();
	
	TFile* outFile_y = new TFile("media/root_files/Younes_NTuples_TOTEM20/glueball_mass_reconstruction/rho_y_proj.root", "RECREATE");
	y_projection->Draw("E*");
	fit_y->Draw("SAME");
    canvas->Update();
	canvas->Write();
	...
	//X projections
	Double_t y_1 = 650, y_2 = 950; // MeV/c^2
	...
	fit_x->Draw("SAME");
	canvas->Update();
	canvas->SaveAs("media/photos/Younes_NTuples_TOTEM20/glueball_mass_reconstruction/rho_x_proj.pdf");
	canvas->Clear();
	
	TFile* outFile_x = new TFile("media/root_files/Younes_NTuples_TOTEM20/glueball_mass_reconstruction/rho_x_proj.root", "RECREATE");
	x_projection->Draw("E*");
	fit_x->Draw("SAME");
    canvas->Update();
	canvas->Write();
