...
int main(){
	...
	y_projection->GetYaxis()->SetTitle(Form("Events / %.2g MeV)",(1.0*y_projection->GetBinWidth())));
	...
	fit_y->Draw("SAME");
	canvas->Update();
	canvas->SaveAs("media/photos/Younes_NTuples_TOTEM20/glueball_mass_reconstruction/kaons_from_rho_y_proj.pdf");
	canvas->Clear();
	
	TFile* outFile_y = new TFile("media/root_files/Younes_NTuples_TOTEM20/glueball_mass_reconstruction/kaons_from_rho_y_proj.root", "RECREATE");
	y_projection->Draw("E*");
	fit_y->Draw("SAME");
	legend_y->Draw();
    canvas->Update();
	...
	x_projection->GetYaxis()->SetTitle(Form("Events (.%1f)",(Float_t)((y_bin_2 - y_bin_1)/(y_bins * 1.0))));
	...
	fit_x->Draw("SAME");
	canvas->Update();
	canvas->SaveAs("media/photos/Younes_NTuples_TOTEM20/glueball_mass_reconstruction/kaons_from_rho_x_proj.pdf");
	canvas->Clear();
	
	TFile* outFile_x = new TFile("media/root_files/Younes_NTuples_TOTEM20/glueball_mass_reconstruction/kaons_from_rho_x_proj.root", "RECREATE");
	x_projection->Draw("E*");
	fit_x->Draw("SAME");
    canvas->Update();
	canvas->Write();
