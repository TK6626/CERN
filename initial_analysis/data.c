...
void data()
{
	...
	TH2F* prot_an_x = new TH2F("prot_an_x", "prot_an_x", 1000, -2e-3, 2e-3, 1000, -2e-3, 2e-3);
	TH2F* prot_an_y = new TH2F("prot_an_y", "prot_an_y", 1000, -2e-3, 2e-3, 1000, -2e-3, 2e-3);
	...
	// Obtain an index to cycle through the entries
	cout<<"entries = "<<entries<<endl;

	for(Int_t e=0; e<entries; ++e)   
	{
		// Gets a given value for each of the branches at a given entry
		tree->GetEntry(e);


		// Create a counter for the number of protons and the number of positive and negative pions
		n_p=0;
		n_m=0;
		
		// get total x and y momentum of the 4 pions 
		px_reco = 0;
		py_reco = 0;
		
		// instantiate angle the net momentum of the 4-track-muons makes to the beam line at interaction point
		// i.e. this is theta*
		x_angle = 0;
		y_angle = 0;
			
		// Choose number of tracks observed - wee look for 4-pion decay
		if(ntrk == 4) //selecting 4-tracks
		{
			// assume that the transverse momentum is not low - but we need to instantiate it 
			pt_low = false;
			
			// Kepp a counter for the number of 4-track (assume to be muon) events we observe in this set of data
			n_4trk++;
			

			// Loop through each of the (4) tracks to obtain kinematical variables for the (assumed to be) pions
			for(Int_t j=0; j < ntrk; ++j)	
			{	
				// Check that the event has the necessary low transverse momentum
				// if not then reject (in MeV?)
				if(trk_pt[j] < 0.25) pt_low = true;
				

				// Use roman pots to determine the scattering angles of the protons and use for later
				// We get the proton_scattering_angle in the x and the y directions from the detercotrs
				prot_an_x->Fill(ThxL, ThxR);
				prot_an_y->Fill(ThyL, ThyR);

				
				
				// Calculate the total scattering angle in the x and y direction (why they are
				// labled Left and right is beyond me) (x is pointed inwards towards the ring centre, y is pointed upwards
				// towards the sky)
				x_angle = ThxL + ThxR;
				y_angle = ThyL + ThyR;
				
				// Get total(magnitude of) momentum of each pion (track)
				p = trk_pt[j]*cosh(trk_eta[j]);
				
				// fill the de/dx with the momentum just calculated and the energy loss of the track (of the pion)
				dedx->Fill(p, trk_dedx[j]);

				// Calculate all other kinematic variables useful
				px = trk_pt[j]*cos(trk_phi[j]);
				py = trk_pt[j]*sin(trk_phi[j]);
				pz = trk_pt[j]*sinh(trk_eta[j]);
				
				// reconstruct the total x and y momentum of the initial state
				// (i.e. the momentum of the initial bound state)
				px_reco += trk_pt[j]*cos(trk_phi[j]);
				py_reco += trk_pt[j]*sin(trk_phi[j]);
				
				// Get total energy of the each pion
				E = sqrt(pion_mass*pion_mass + trk_pt[j]*trk_pt[j] + pz*pz);
				
				// Just go by each positive and negatice pions to ensure charge balance and set 4-momentum values
			...
			for(Int_t a=0; a < 4; ++a)	
			{	
				// Form all possible pairs of pions combinations
				// Need to loop through all the differnt combinations it is possible to form while maintaining
				// charge conservation
			...
			// Calculate invarient mass of the resonances
			...
			// Fill histogram of masses
			m->Fill(p11[4], p22[4]);
			m->Fill(p12[4], p21[4]);

			// if transverse moentum is not low then fill another graph
			if(!pt_low) 
			{
			...
			// Nominal momentum of the each proton in Beam (MeV)
			p_x = x_angle*6500;
			p_y = y_angle*6500;
			

			// Compare angles between the reoconstructed momentum of the pions, and the polar angle of the protons
			th_px->Fill(px_reco, x_angle);
			th_py->Fill(py_reco, y_angle);
			
			// Compare angles between the reoconstructed momentum of both the pions, and the protons
			th_px1->Fill(px_reco, p_x);
			th_py1->Fill(py_reco, p_y);
		}
				
	}
	std::cout << "prot_an_x entries: " << prot_an_x->GetEntries() << std::endl;
	std::cout << "prot_an_y entries: " << prot_an_y->GetEntries() << std::endl;
